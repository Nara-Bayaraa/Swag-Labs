...
describe("UI: Login Functionality - Negative Tests", () => {
  beforeEach(() => {
    cy.visit("/");
  });

  it("[LOGIN-UI-NEG-001] should show correct error for valid username and invalid password", () => {
  ...
  describe("Scenario: Invalid Username and Valid Password", () => {
    ...
    it("[LOGIN-UI-NEG-OO2] should show an error message", () => {
      LoginPage.assertLoginErrorMessageIsDisplayed(
  ...
  it("[LOGIN-UI-NEG-004] should show 'Password is required' for valid username and empty password", () => {
    LoginPage.login(Cypress.env("standardUsername"), "");
    LoginPage.assertLoginErrorMessageIsDisplayed("Password is required");
  });

  it("[LOGIN-UI-NEG-005] should show 'Username is required' for empty username and valid password", () => {
    LoginPage.login("", Cypress.env("standardPassword"));
    LoginPage.assertLoginErrorMessageIsDisplayed("Username is required");
  });

  it("[LOGIN-UI-NEG-006] should show 'Username is required' for empty username and empty password", () => {
    LoginPage.login("", "");
    LoginPage.assertLoginErrorMessageIsDisplayed(
